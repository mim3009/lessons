<?php
	...
	class DataBase{
		...
		public function getField($table_name, $field_out, $filed_in, $value_in){
			$data = $this->select($table_name, array($field_out), "`$filed_in` = '" . addslashes($value_in) . "'");
			if(count($data) != 1){
				return false;
			}
		...
		public function maxValue($table_name, $field){
			$data = $this->select($table_name, array("MAX(`$field`)"));
			return $data[0]["MAX(`id`)"];
		}

		public function minValue($table_name, $field){
			$data = $this->select($table_name, array("MIN(`$field`)"));
			return $data[0]["MIN(`$field`)"];
		}

		public function beetwen($table_name, $value, $l_interval, $r_interval){
			return $this->select($table_name, array("*"), "`$value` BETWEEN $l_interval AND $r_interval");
		}
